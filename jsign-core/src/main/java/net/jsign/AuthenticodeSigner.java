 ...
 * signer.withProgramName("My Application").withProgramURL("http://www.example.com").withTimestamping(true)
...
public class AuthenticodeSigner {
	 ...
	 */
	public AuthenticodeSigner(KeyStore keystore, String alias, String password)
    ...
    protected CMSSignedData createSignedData(Signable file) throws Exception {
    	...
    	if (file instanceof NugetFile) {
            sigData = generator.generate(new CMSProcessableByteArray(PKCSObjectIdentifiers.signedData, file.computeDigest(digestAlgorithm)), true);
    	}
    	else {
	...
	private CMSSignedDataGenerator createSignedDataGenerator()
			throws CMSException, OperatorCreationException, CertificateEncodingException {
	...
	private SignerInfoGenerator createSignerInfoGenerator()
		...
		attributeTableGenerator = new FilteredAttributeTableGenerator(attributeTableGenerator,
				CMSAttributes.signingTime, CMSAttributes.cmsAlgorithmProtect);
					...
					public AlgorithmIdentifier findEncryptionAlgorithm(final AlgorithmIdentifier signatureAlgorithm) {
						...
						if (signatureAlgorithm.getAlgorithm().equals(PKCSObjectIdentifiers.sha256WithRSAEncryption)
								|| signatureAlgorithm.getAlgorithm()
										.equals(PKCSObjectIdentifiers.sha384WithRSAEncryption)
								|| signatureAlgorithm.getAlgorithm()
										.equals(PKCSObjectIdentifiers.sha512WithRSAEncryption)) {
		...
		signerInfoGeneratorBuilder
				.setContentDigest(createContentDigestAlgorithmIdentifier(shaSigner.getAlgorithmIdentifier()));
	...
	private void verify(CMSSignedData signedData) throws SignatureException, OperatorCreationException {
		...
		DigestCalculatorProvider digestCalculatorProvider = new JcaDigestCalculatorProviderBuilder().build();
		SignerInformationVerifier verifier = new JcaSignerInfoVerifierBuilder(digestCalculatorProvider)
				.build(publicKey);
		...
		if (!result) {
			throw new SignatureException("Signature verification failed, the private key doesn't match the certificate",
					cause);
		}
	}
	...
	private AttributeTable createAuthenticatedAttributes() {
		...
		SpcStatementType spcStatementType = new SpcStatementType(
				AuthenticodeObjectIdentifiers.SPC_INDIVIDUAL_SP_KEY_PURPOSE_OBJID);
		attributes.add(
				new Attribute(AuthenticodeObjectIdentifiers.SPC_STATEMENT_TYPE_OBJID, new DERSet(spcStatementType)));
	...
	protected CMSSignedData addNestedSignature(CMSSignedData primary, CMSSignedData secondary) {
		...
		Attribute nestedSignaturesAttribute = unsignedAttributes
				.get(AuthenticodeObjectIdentifiers.SPC_NESTED_SIGNATURE_OBJID);
		if (nestedSignaturesAttribute == null) {
			// first nested signature
			unsignedAttributes = unsignedAttributes.add(AuthenticodeObjectIdentifiers.SPC_NESTED_SIGNATURE_OBJID,
					secondary.toASN1Structure());
		} else {
			...
			ASN1EncodableVector attributes = unsignedAttributes
					.remove(AuthenticodeObjectIdentifiers.SPC_NESTED_SIGNATURE_OBJID).toASN1EncodableVector();
			attributes.add(new Attribute(AuthenticodeObjectIdentifiers.SPC_NESTED_SIGNATURE_OBJID,
					new DERSet(nestedSignatures)));
